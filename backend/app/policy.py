...
class EvidenceGate:
    read_actions: ClassVar[set[str]] = {
        "get_change_request", "get_topology", "get_service_health", "get_metrics",
        "get_recent_logs", "search_runbooks", "search_maintenance_history",
        "check_capacity", "evaluate_evidence",
    }
    safe_actions: ClassVar[set[str]] = {
        "create_snapshot", "drain_node", "create_rollback_point", "apply_maintenance",
        "restart_service", "run_health_check", "run_synthetic_test",
        "restore_node_to_pool", "create_maintenance_report", "defer_change",
    }
    rollback_actions: ClassVar[set[str]] = {"rollback_change", "verify_rollback"}
    restricted_actions: ClassVar[set[str]] = {
        "delete_database", "delete_backup", "disable_auditing", "modify_credentials",
        "disable_security_controls", "destructive_schema_change", "arbitrary_shell",
    }
    ...
    def evaluate(
        ...
        if gate == "maintenance_window":
            ...
            evidence = [
                EvidenceItem(key="approved", label="Change request approved", passed=approved,
                             observed=approved, required=True),
                *[
                    EvidenceItem(key=f"{node.id}_health", label=f"{node.name} health",
                                 passed=node.health == HealthState.HEALTHY,
                                 observed=node.health, required=HealthState.HEALTHY)
                    for node in nodes
                ],
                EvidenceItem(key="minimum_capacity", label="Minimum web capacity",
                             passed=capacity["passed"], observed=capacity["available_capacity"],
                             required=capacity["required_capacity"]),
                EvidenceItem(key="backup_available", label="Recent backup available",
                             passed=snapshot["verified"] and snapshot["age_minutes"] < 60,
                             observed=snapshot["age_minutes"], required="verified and < 60 minutes"),
            ]
        elif gate == "drain_node":
            capacity = self.simulator.check_capacity(excluding=target)
            evidence = [EvidenceItem(
                key="remaining_capacity", label="Remaining web capacity",
                passed=capacity["passed"], observed=capacity["available_capacity"],
                required=capacity["required_capacity"],
            )]
            ...
            evidence = [
                EvidenceItem(key="node_drained", label="Node drained",
                             passed=not node.in_load_balancer,
                             observed=node.in_load_balancer, required=False),
                EvidenceItem(key="rollback_point", label="Rollback point verified",
                             passed=rollback_exists, observed=rollback_exists, required=True),
            ]
        elif gate == "rollback":
            node = self.simulator.get(target)
            rollback_exists = self.simulator.has_rollback_point(maintenance_id, target)
            evidence = [
                EvidenceItem(key="rollback_point", label="Rollback point exists",
                             passed=rollback_exists, observed=rollback_exists, required=True),
                EvidenceItem(key="change_owner", label="Current maintenance owns change",
                             passed=node.active_change_id == maintenance_id,
                             observed=node.active_change_id, required=maintenance_id),
            ]
            ...
            snapshot = self.simulator.latest_snapshot()
            evidence = [
                EvidenceItem(key="recent_backup", label="Recent backup exists",
                             passed=snapshot["verified"], observed=snapshot["age_minutes"],
                             required="verified"),
                EvidenceItem(key="database_health", label="Database currently healthy",
                             passed=database.health == HealthState.HEALTHY,
                             observed=database.health, required=HealthState.HEALTHY),
                EvidenceItem(key="web01_target", label="WEB01 desired version healthy",
                ...
                EvidenceItem(key="web02_target", label="WEB02 desired version healthy",
                ...
                EvidenceItem(key="full_redundancy", label="Web tier fully redundant",
                             passed=web01.in_load_balancer and web02.in_load_balancer
                             and web01.version == web01.desired_version
                             and web02.version == web02.desired_version,
                             observed={"web01": web01.in_load_balancer,
                                       "web02": web02.in_load_balancer},
                             required="two healthy target-version nodes"),
                EvidenceItem(key="global_error_rate", label="Global application error rate",
                             passed=self.simulator.global_error_rate() <= 5.0,
                             observed=self.simulator.global_error_rate(), required="<= 5%"),
            ]
        else:
            evidence = [EvidenceItem(key="unknown_gate", label="Known evidence gate",
                                     passed=False, observed=gate, required="registered gate")]
        ...
        return GateDecision(
            ...
            summary=(f"{gate} evidence satisfied" if passed
