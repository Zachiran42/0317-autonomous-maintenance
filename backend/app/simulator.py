...
class Simulator:
        ...
        with self._lock:
            self._nodes = {
                "load-balancer": InfrastructureNode(
                    id="load-balancer", name="Load Balancer", kind="load_balancer",
                    version="2.3.0", desired_version="2.3.0", capacity_units=100,
                    dependencies=["web01", "web02"],
                ),
                "web01": InfrastructureNode(
                    id="web01", name="WEB01", kind="web",
                    dependencies=["worker", "database"],
                    logs=["INFO web01 serving traffic normally"],
                ),
                "web02": InfrastructureNode(
                    id="web02", name="WEB02", kind="web",
                    dependencies=["worker", "database"],
                    logs=["INFO web02 serving traffic normally"],
                    injected_fault="configuration_incompatible_after_update",
                ),
                "worker": InfrastructureNode(
                    id="worker", name="API / Worker", kind="worker", version="4.2.1",
                    desired_version="4.2.1", in_load_balancer=False, dependencies=["database"],
                ),
                "database": InfrastructureNode(
                    id="database", name="Database", kind="database", version="15.4",
                    desired_version="15.5", in_load_balancer=False, latency_ms=18,
                    capacity_units=100,
                ),
            }
    ...
    def global_error_rate(self) -> float:
        with self._lock:
            active = [self._nodes[node] for node in ("web01", "web02") if self._nodes[node].in_load_balancer]
    ...
    def check_capacity(self, excluding: str | None = None) -> dict[str, Any]:
        with self._lock:
            active = [
                self._nodes[node] for node in ("web01", "web02")
                if node != excluding and self._nodes[node].in_load_balancer
                and self._nodes[node].health == HealthState.HEALTHY
            ]
            capacity = sum(node.capacity_units for node in active)
            return {"available_capacity": capacity, "required_capacity": 50, "passed": capacity >= 50}
    ...
    def create_snapshot(self, maintenance_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            self._snapshots[key] = {
                "id": key, "target": "database", "verified": True,
                "age_minutes": 0, "version": self._nodes["database"].version,
            }
    ...
    def create_rollback_point(self, maintenance_id: str, node_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            self._rollback_points[key] = {
                "maintenance_id": maintenance_id, "target": node_id, "version": node.version,
                "health": node.health, "error_rate": node.error_rate,
                "latency_ms": node.latency_ms, "verified": True,
            }
    ...
    def run_health_check(self, node_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            node.latency_ms = 52
            return {"target": node_id, "passed": True, "health": node.health, "latency_ms": node.latency_ms}
    ...
    def run_synthetic_test(self, node_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            if node.injected_fault and node.active_change_id:
                ...
                return {
                    "target": node_id, "passed": False, "error_rate": node.error_rate,
                    "threshold": 5.0, "reason": "Functional verification failed after update",
                }
            node.health = HealthState.HEALTHY
            node.error_rate = 0.2
            node.latency_ms = 47
            return {"target": node_id, "passed": True, "error_rate": node.error_rate, "threshold": 5.0}
    ...
    def verify_rollback(self, maintenance_id: str, node_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            node = self._nodes[node_id]
            passed = bool(snapshot and node.version == snapshot["version"]
                          and node.health == HealthState.HEALTHY and node.in_load_balancer)
            return {
