...
class MaintenanceTools:
    ...
    def _call(
        ...
        *,
        ...
        log_event("tool_call", maintenance_id=self.maintenance_id, action_id=action_id,
                  tool=name, target=target, status="started")
        try:
            result = structured(fn())
            if idempotent:
                self.repository.save_action(ActionExecution(
                    action_id=action_id,
                    maintenance_id=self.maintenance_id,
                    idempotency_key=key,
                    action=name,
            ...
            log_event("tool_call", maintenance_id=self.maintenance_id, action_id=action_id,
                      tool=name, target=target, status="success")
            return result
            ...
            log_event("tool_call", maintenance_id=self.maintenance_id, action_id=action_id,
                      tool=name, target=target, status="error")
            raise
    ...
    def search_runbooks(self, query: str) -> dict[str, Any]:
        ...
        def search() -> dict[str, Any]:
            ...
            if "web" in text or "rolling" in text:
                matches.append({"id": "RB-ROLLING-12", "title": "Zero-downtime web rolling update",
                                "requires": ["capacity", "drain", "rollback point", "synthetic test"]})
            if "database" in text:
                matches.append({"id": "RB-DB-08", "title": "Evidence-gated database maintenance",
    ...
    def search_maintenance_history(self, query: str) -> dict[str, Any]:
        ...
        return self._call("search_maintenance_history", "history", lambda: {
            "matches": [
                {"maintenance_id": run.id, "status": run.status,
    ...
    def check_capacity(self, excluding: str | None = None) -> dict[str, Any]:
        ...
        return self._call("check_capacity", excluding or "web-tier",
    ...
    def create_snapshot(self, target: str = "database") -> dict[str, Any]:
        ...
        return self._call("create_snapshot", target,
    ...
    def create_rollback_point(self, target: str) -> dict[str, Any]:
        ...
        return self._call("create_rollback_point", target,
                          lambda: self.simulator.create_rollback_point(
                              self.maintenance_id or "", target), idempotent=True)
    ...
    def apply_maintenance(self, target: str) -> dict[str, Any]:
        ...
        def apply() -> Any:
            decision = self.gate.evaluate(
                "apply_web_change", target, self.maintenance_id or ""
            )
            self.gate.enforce_gate(decision)
    ...
    def restart_service(self, target: str) -> dict[str, Any]:
        ...
        return self._call("restart_service", target,
                          lambda: self.simulator.restart_service(target), idempotent=True)

    def run_health_check(self, target: str) -> dict[str, Any]:
        """Run a readiness/health verification against a target."""
        return self._call("run_health_check", target,
                          lambda: self.simulator.run_health_check(target))
    ...
    def run_synthetic_test(self, target: str) -> dict[str, Any]:
        ...
        return self._call("run_synthetic_test", target,
                          lambda: self.simulator.run_synthetic_test(target))

    def restore_node_to_pool(self, target: str) -> dict[str, Any]:
        """Restore a verified healthy node to the simulated load balancer."""
        return self._call("restore_node_to_pool", target,
                          lambda: self.simulator.restore_node_to_pool(target), idempotent=True)
    ...
    def verify_rollback(self, target: str) -> dict[str, Any]:
        ...
        return self._call("verify_rollback", target,
                          lambda: self.simulator.verify_rollback(
                              self.maintenance_id or "", target))
