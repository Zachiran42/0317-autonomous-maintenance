param(
    ...
    [string]$ServiceName = "incident-response-agent"
)
...
gcloud pubsub topics describe incident-events --project $ProjectId 2>$null
if ($LASTEXITCODE -ne 0) { gcloud pubsub topics create incident-events --project $ProjectId }
...
gcloud run deploy $ServiceName `
    ...
    --cpu 1 `
    --timeout 300 `
    --set-env-vars "ENVIRONMENT=production,AGENT_RUNTIME=adk,PERSISTENCE_BACKEND=firestore,EVENT_BACKEND=pubsub,GOOGLE_CLOUD_PROJECT=$ProjectId,GOOGLE_CLOUD_LOCATION=global,GOOGLE_GENAI_USE_VERTEXAI=true,GEMINI_MODEL=gemini-3.5-flash,PUBSUB_TOPIC=incident-events"

$serviceUrl = gcloud run services describe $ServiceName --region $Region --format="value(status.url)"
$subscription = "incident-worker"
...
if ($LASTEXITCODE -eq 0) {
    ...
    gcloud pubsub subscriptions create $subscription --topic=incident-events --push-endpoint="$serviceUrl/api/events/pubsub" --ack-deadline=300 --project $ProjectId
}
