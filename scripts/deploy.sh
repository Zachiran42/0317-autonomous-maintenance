...
gcloud pubsub topics describe incident-events >/dev/null 2>&1 || gcloud pubsub topics create incident-events
...
gcloud run deploy "$SERVICE_NAME" --source . --region "$REGION" --allow-unauthenticated --min-instances 0 --max-instances 2 --memory 1Gi --cpu 1 --timeout 300 --set-env-vars "ENVIRONMENT=production,AGENT_RUNTIME=adk,PERSISTENCE_BACKEND=firestore,EVENT_BACKEND=pubsub,GOOGLE_CLOUD_PROJECT=$PROJECT_ID,GOOGLE_CLOUD_LOCATION=global,GOOGLE_GENAI_USE_VERTEXAI=true,GEMINI_MODEL=gemini-3.5-flash,PUBSUB_TOPIC=incident-events"
SERVICE_URL="$(gcloud run services describe "$SERVICE_NAME" --region "$REGION" --format='value(status.url)')"
gcloud pubsub subscriptions describe incident-worker >/dev/null 2>&1 && gcloud pubsub subscriptions update incident-worker --push-endpoint="$SERVICE_URL/api/events/pubsub" || gcloud pubsub subscriptions create incident-worker --topic=incident-events --push-endpoint="$SERVICE_URL/api/events/pubsub" --ack-deadline=300
